function initiate() {
    ...
    $.get("order.php?filter=accepted", function(data, status){
        ...
        if(status == "nocontent")
            ...
            $('#orders').append("<h3>Orders</h3><hr><table class='table table-striped'><thead><tr><th>#</th><th>Customer ID</th><th>Date&time</th><th>Seats</th><th>Price</th><th>Confirmation</th></tr></thead><tbody id='td'></tbody><table>");
            for(var i=0; i<data.length; i++) {
                console.log("somethin");
                $('#td').append("<tr><td>"+(i+1)+"</td><td>"+data[i][1]+"</td><td>"+data[i][3]+"</td><td>"+data[i][6]+"</td><td>"+data[i][4]+"</td><td><a href='#' class='btn btn-success' onclick='confirmOrder("+data[i][0]+")'>Confirm</a></td></tr>")
            }
        }
...
function confirmOrder(order_id) {
    $.get("order.php?action=confirm&order_id="+order_id, function(data, status){
        initiate();
    });
}

function acceptOrder(order_id) {
    $.get("order.php?action=accept&order_id="+order_id, function(data, status){
        requestsClicked();
    });
}

function rejectOrder(order_id) {
    $.get("order.php?action=reject&order_id="+order_id, function(data, status){
        requestsClicked();
    });
}